...
<?php else: ?>
  <div class="panel-grid horarios-grid-wrap"><table class="horarios-grid"><thead><tr><th>Tramo</th><th>Lunes</th><th>Martes</th><th>Miércoles</th><th>Jueves</th><th>Viernes</th></tr></thead><tbody><?php foreach ($tramos as $tramo): ?><tr><td><?php echo (int) $tramo['numero_tramo']; ?>ª<?php if (!empty($tramo['hora_inicio']) && !empty($tramo['hora_fin'])): ?> (<?php echo htmlspecialchars(substr((string) $tramo['hora_inicio'], 0, 5), ENT_QUOTES, 'UTF-8'); ?>-<?php echo htmlspecialchars(substr((string) $tramo['hora_fin'], 0, 5), ENT_QUOTES, 'UTF-8'); ?>)<?php endif; ?></td><?php for ($day = 1; $day <= 5; $day++): $key = $day . '-' . (int) $tramo['id_horario_tramo']; $cell = $scheduleMap[$key] ?? null; ?><td><?php if ($cell !== null): $moduleId = (int) $cell['id_modulo']; $colorClass = $moduleColorMap[$moduleId] ?? 'module-color-disabled'; ?><div class="horario-cell-module <?php echo htmlspecialchars($colorClass, ENT_QUOTES, 'UTF-8'); ?>"><span><?php echo htmlspecialchars((string) $cell['nombre'], ENT_QUOTES, 'UTF-8'); ?></span></div><?php endif; ?></td><?php endfor; ?></tr><?php endforeach; ?></tbody></table></div>
<?php endif; ?>
</section>
</main></div></body></html>
